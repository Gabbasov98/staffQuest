...
function setLabelPlace(){
    let fieldBlocks = document.querySelectorAll(".fg-dynamic")

    fieldBlocks.forEach(block => {
        let field = block.querySelector("input,textarea,select")
        if(!field){
            return
        }
...
filterSelects.forEach(select => {
    ...
    let current = select.querySelector(".filter-select__field")
    let current2 = select.querySelector(".filter-select__current")
    ...
    function setCurrentValue() {
        ...
        let id = checkedOption.getAttribute("data-id")
        current.value = value
        current2.innerHTML = value
        select.classList.add("_selected")
        $(".vacancies__content").removeClass("_active")
        $(`.vacancies__content.${id}`).addClass("_active")
    }
})
